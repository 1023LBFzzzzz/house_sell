...
citySelector.hotCity[0] =  new Array("南京", "玄武区|秦淮区|鼓楼区|建邺区|雨花台区|栖霞区|浦口区|溧水区|江宁区");
...
for (var i = 0; i < citySelector.hotCity.length; i++) {
  citySelector.hotCityhtmls +=  "<li data-xuhao='" + i + "'><span class='provinceName'>" + citySelector.hotCity[i][0] + "</span></li>";
}
...
citySelector.cityInit = function (input) {
  ...
  $("#" + input).click(function () {
    ...
    $("#js_provCity").on("click", ".provinceName", function () {
      function createUl(_this){
        _this.css({ "background": "#fff", "border-color": "#d8d8d8", "border-bottom-color": "#fff",/* "position": "absolute",*/ "top": "0", "left": "0", "z-index": "999999" });
        var xuhao = _this.parent("li").attr("data-xuhao"),
          cityArr = citySelector.hotCity[xuhao][1].split("|"),
          cityHtmls = "<ul id='js_provCitys'>";
        for (var i = 0; i < cityArr.length; i++) {
          cityHtmls += "<li class='js_cityName'>" + cityArr[i] + "</li>";
        }
      ...
      if (label == "false") {
        ...
        if ($(this).attr("now-item") == "" || $(this).attr("now-item") == "false" || $(this).attr("now-item") == undefined) {
          $(this).parents("#js_provCity").find("span").attr("now-item", "false");
          $(this).attr("now-item", "true");
          $("#js_provCity span").css({ "background": "", "border-color": "", "border-bottom-color": "", "position": "", "top": "", "left": "", "z-index": "" });
          $("#js_provCitys").remove();
          createUl($(this));
        }
        else {
    ...
    $("#js_cityBox").on("click", ".js_cityName", function (e) {
      e.stopPropagation();
      $("#" + _input).val($(this).html()).change();
      /*  $("#" + _input).value = $(this).html();*/
      var provinceObj = document.getElementById("inputTest");
